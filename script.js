...
btn_add.addEventListener("click", function() {
  ...
  if (arr_notas.length >= 15) {
    ...
    let html_li =
      '<div class="row"><label for="" class="col-form-label col-3">Nota ' +
      (arr_notas.length + 1) +
      '</label><div class="form-group col-3 col p-1" ><input type="text" id="' +
      (arr_notas.length - 1) +
      '" class="input-nota form-control" value="0" placeholder="Nota"></div><label for="" class="col-form-label col-1"> = </label><div class="form-group col-3 col p-1" ><input type="text" id="' +
      (arr_porcentaje.length - 1) +
      '" class="input-porcentaje form-control" value="0" placeholder="Porcentaje"></div><label for="" class="col-form-label col-1"> % </label></div>';
...
function events(elementos) {
  elementos.addEventListener("blur", function() {
    if (elementos.value == "" || isNaN(elementos.value)) {
      ...
      if (
        elementos.id.match(/input-nota-examen*/) ||
        elementos.id.match(/input-porcentaje-examen*/)
      ) {
...
function obtenerPromedio() {
  ...
  if (porcentaje_total !== 0) {
    arr_notas.forEach(function(w, ind) {
      promedio +=
        parseInt(arr_notas[ind].value) *
        parseInt(arr_porcentaje[ind].value) /
        porcentaje_total;
    });
...
btn_dar_examen.addEventListener("click", function() {
  ...
  if (
    document.getElementById("contenido-examen").classList.contains("d-none")
  ) {
...
function obtenerPromedioExamen() {
  ...
  if (porcentaje_total !== 0) {
    promedio_examen =
      promedio * (100 - parseInt(porcentaje_examen.value)) / porcentaje_total +
      parseInt(nota_examen.value) *
        parseInt(porcentaje_examen.value) /
        porcentaje_total;
